...
4. Acts on that decision — cleaning up on **Continue**, or reprocessing and re-reviewing on **Re-process**. Each reprocess attempt gets its own output directory (`{id}_processed_{timestamp}_attempt{N}`), so prior attempts and their previews aren't overwritten and the reviewer always sees the image that matches the attempt actually being reviewed. A **Re-process** decision is honored only if the campaign hasn't already given up waiting for storage to free (tracked via a Prefect `Variable` set when `main()`'s retry loop hits its give-up branch, not a point-in-time byte count — a single unrelated quarantine can transiently drop the tracked total below threshold without meaning the campaign can actually sustain more reprocessing) — otherwise it's quarantined immediately instead of writing more output. If `quality_gate.max_attempts` is exhausted without a **Continue** (or the campaign has already given up), the observation is marked `FAILED` and both its raw `.ms` and every attempt's processed output are moved (not deleted) into `storage.failed_dir` for manual inspection — this also keeps them from permanently counting toward the storage threshold, which would otherwise stall the campaign forever waiting on space that was never coming.
